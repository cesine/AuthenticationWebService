 ...
 * Required
 */
model.getAccessToken = function getAccessToken(bearerToken, callback) {
...
model.getRefreshToken = function getRefreshToken(bearerToken, callback) {
  pg.connect(connString, function (err, client, done) {
    if (err) return callback(err);
...
model.grantTypeAllowed = function grantTypeAllowed(clientId, grantType, callback) {
  if (grantType === 'password') {
    return callback(false, authorizedClientIds.indexOf(clientId.toLowerCase()) >= 0);
  }
  callback(false, true);
};
model.saveAccessToken = function saveAccessToken(accessToken, clientId, expires, userId, callback) {
  pg.connect(connString, function (err, client, done) {
    if (err) return callback(err);
...
model.saveRefreshToken = function saveRefreshToken(refreshToken, clientId, userId, expires, callback) {
  pg.connect(connString, function (err, client, done) {
    if (err) return callback(err);
 ...
 */
model.getUser = function getUser(username, password, callback) {
  pg.connect(connString, function (err, client, done) {
    if (err) return callback(err);
