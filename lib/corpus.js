...
var createPlaceholderDocsForCorpus = function(corpusObject) {
  ...
  corpusDetails.confidential.secretkey = Confidential.secretKeyGenerator();
...
var createNewCorpus = function(corpusObject, done) {
  ...
  server.db.create(corpusObject.dbname, function(couchDBError, response) {
    ...
    newDatabase.bulk({
      ...
      if (couchDBError) {
        console.log(new Date() + " There was an couchDBErroror in creating the docs for the user\'s new corpus: " + util.inspect(couchDBError) + "\n");
    ...
    if (!couchDBError) {
      ...
      db.insert(userold, function(couchDBError, couchresponsetochangingusersroles) {
        if (!couchDBError) {
          ...
          console.log(new Date() + " User roles " + userid + "failed to update.", couchDBError, couchresponsetochangingusersroles);
        }
      });
    } else {
      console.log(new Date() + " failed to change " + username + " user's roles due to couchdb error ", roles, couchDBError);
    }
  });
};
