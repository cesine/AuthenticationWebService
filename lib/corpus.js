...
var createNewCorpus = function(req, done) {
  ...
  var connection = new Connection(Connection.defaultConnection(appbrand));
  connection.title = req.body.newCorpusName;
  connection.dbname = username + "-" + connection.titleAsUrl;
  console.log(new Date() + " Creating new database " + connection.dbname);

  var server = require('nano')(couchConnectUrl);
  server.db.create(connection.dbname, function(err, response) {
    if (err) {
      console.log(err);
    ...
    console.log("Created db " + connection.dbname, response);
    /*
    ...
    var newDatabase = require('nano')(couchConnectUrl + "/" + connection.dbname);
    ...
    server.db.replicate('new_corpus', connection.dbname, function(err, couchResponse) {
      if (!err) {
    ...
    var corpusDetails = new Corpus({
      dbname: connection.dbname,
      title: req.body.newCorpusName,
      connection: connection,
      id: uuid.v4()
    });

    connection.corpusid = corpusDetails.id;
    ...
    var docsNeededForAProperFieldDBCorpus = [team.toJSON(), corpusMaskDetails.toJSON(), corpusDetails.toJSON(), datalistDetails.toJSON(), sessionDetails.toJSON()];
    ...
    newDatabase.bulk({
      ...
      if (err) {
        ...
        // undoCorpusCreation(user, corpusDetails.connection, docsNeededForAProperFieldDBCorpus);
      } else {
        console.log(new Date() + " Created starter docs for " + corpusDetails.dbname + "\n", couchResponse);
      }
    });

    // Replicate activity feed template to new activity feed database
    server.db.replicate('new_corpus_activity_feed', connection.dbname + "-activity_feed", {
      create_target: true
      ...
      if (!err) {
        ...
        var activityDb = require('nano')(couchConnectUrl + "/" + newActivityFeedConnect);
    ...
    return done(null, corpusDetails, {});
  });
};
