...
module.exports.createDbaddUser = function(dbConnection, user, successcallback, errorcallback) {
  ...
  user.appbrand = user.appbrand || "production";
  var app = node_config.apps[user.appbrand];
  if (!app) {
    app = node_config.apps.production;
  }
  ...
  var userParamsForNewUser = {
    ...
    roles: [dbConnection.dbname + "_" + admin,
       ...
       * to collaborate
       */
      app.sampleCorpus + "_" + contributor,
      app.sampleCorpus + "_" + collaborator,
      app.sampleCorpus + "_" + commenter,
      "public-firstcorpus" + "_" + collaborator,
      "fielddbuser",
      app.brandLowerCase + "user"
    ],
    type: 'user'
  };
  usersdb.save(userid, userParamsForNewUser, function(err, doc) {
    ...
    if (err !== null || !doc.ok) {
       ...
       */
      createDBforUsersActivities(user.activityConnection, user, function(res) {
  ...
  db.exists(function(err, exists) {
    if (err) {
      ...
      db.create(function(err, res) {
        console.log(new Date() + " In the callback of db create for " + dbConnection.dbname);
        if (err) {
