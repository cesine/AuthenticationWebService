    ...
    if (err) {
      ...
      db.create(function (err, couchdbresponse) {
        ...
        c.replicate({
          ...
          if (err) {
            console.log(new Date() + "unable to replicate the map reduces to the new user's database " + dbConnection.dbname, err);
          }
          c.replicate({
            source: 'new_lexicon',
            target: dbConnection.dbname
          }, function (err, result) {
            console.log(new Date() + "replicated the lexicon for the new user's database " + dbConnection.dbname, result);
          });
          // c.replicate({
          //   "source": 'new_export',
          //   "target": dbConnection.dbname
          // }, function(err, result) {
          //   console.log(new Date() + "replicated the export for the new user's database "+ dbConnection.dbname, result);
          // });
          /*
