...
if (!node_config || !node_config.apps || !node_config.apps.production) {
...
module.exports.registerNewUser = function(localOrNot, req, done) {
  ...
  findByUsername(req.body.username, function(err, user, info) {
    if (user) {
      ...
      var connection = req.body.connection || req.body.connection || corpus.getConnectionFromServerCode(serverCode);
      ...
      var private_corpus_template = corpus.createPrivateCorpusDoc(req.body.username, corpusTitle, connection, connection.dbname);
      if (appbrand === "phophlo") {
      ...
      connection.description = private_corpus_template.description;
      private_corpus_template.titleAsUrl = connection.titleAsUrl = corpusTitle.toLowerCase().replace(/[^a-z0-9_]/g, "_");
      ...
      var public_corpus_template = corpus.createPublicCorpusDoc(connection, connection.dbname);

      /* Prepare an empty datalist doc for the user's first corpus */
      var datalist_template = corpus.createDatalistDoc(connection.dbname);

      /* Prepare an empty session doc for the user's first corpus */
      var session_template = corpus.createSessionDoc(connection.dbname);
      ...
      corpus_management.createDbaddUser(userforcouchdb.corpora[0], userforcouchdb, function(res) {
        ...
        var db = require('nano')(couchConnectUrl + "/" + private_corpus_template.dbname);
        db.bulk({
          ...
          if (err) {
            ...
            undoCorpusCreation(user, private_corpus_template.connection, docsNeededForAProperFieldDBDatabase);
            err = err || {};
...
var undoCorpusCreation = function(user, connection, docs) {
  console.log(new Date() + " TODO need to clean up a broken corpus." + util.inspect(connection));
  var email = user.email || "";
  email = email + "";
  if (!email) {
    email = "bounce@lingsync.org";
      ...
      if (req.body.syncDetails == "true") {
        ...
        if (JSON.stringify(user.corpora) != JSON.stringify(req.body.syncUserDetails.corpora)) {
          ...
          var userforcouchdb = {
            ...
            activityConnection: req.body.activityConnection
          };
...
module.exports.fetchCorpusPermissions = function(req, done) {
  ...
  if (req.body.serverCode) {
    dbConn = corpus.getConnectionFromServerCode(req.body.serverCode);
    ...
    if (userPermissionResult.after.length > 0) {
      if (alreadyAdded > -1) {
        ...
        userPermissionResult.message = 'User ' + user.username + ' now has ' +
          ...
          newConnection.dbname + ', the user is already a member of this corpus team.';
        return done(
...
var createNewCorpusesIfDontExist = function(user, userforcouchdb, corpora) {
  ...
  for (var potentialnewcorpus in corpora) {
    corpus_management.createDBforCorpus(
      ...
      function(connectionThatWasCreated) {
        console.log(new Date() + " Added the corpus connection to the user in the database. " + util.inspect(connectionThatWasCreated) + "\n");
      },
      function(connectionThatWasNotCreated, alreadyexisted) {

        if (alreadyexisted == "corpus_existed") {
          console.log(new Date() + " The corpus already existed. " + util.inspect(connectionThatWasNotCreated) + "\n");
        } else {
          console.log(new Date() + " There was an error in creating the new corpus, when the user logs in again, we'll try again. " + util.inspect(connectionThatWasNotCreated) + "\n");
        }
...
for (var app in node_config.apps) {
  if (node_config.apps.hasOwnProperty(app) && node_config.apps[app].sampleUsers && node_config.apps[app].sampleUsers.length > 0) {
    sampleUsers = sampleUsers.concat(node_config.apps[app].sampleUsers);
  }
}
  ...
  usersdb.get(id, function(error, result) {
    if (error) {
      if (error.error == "not_found") {
        ...
        error = error || {};
        error.status = cleanErrorStatus(error.status) || 401;
        return done({
          status: 401,
...
var setPassword = function(oldpassword, newpassword, username, done) {
  ...
  if (username !== safeUsernameForCouchDB) {
    err = err || {};
    err.status = cleanErrorStatus(err.status) || 412;
    return done(err, null, {
      message: 'Username or password is invalid. Please try again.'
    });
  }
