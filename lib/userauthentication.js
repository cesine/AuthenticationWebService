...
module.exports.registerNewUser = function(localOrNot, req, done) {
  ...
  findByUsername(req.body.username, function(err, user, info) {
    ...
    // console.log("this is what the corpora is going to look like ",corpora);
    ...
    // console.log("user values at registration ", user.toJSON());
    ...
    // console.log("this is what will be used to create the team", docsNeededForAProperFieldDBDatabase[0]);
    // console.log("this is what will be used to create the userMask", docsNeededForAProperFieldDBDatabase[1]);
    // console.log("this is what will be used to create the corpusMask", docsNeededForAProperFieldDBDatabase[2]);
    // console.log("this is what will be used to create the corpus", [docsNeededForAProperFieldDBDatabase[3]]);
    // console.log("this is what will be used to create the corpus confidential", docsNeededForAProperFieldDBDatabase[3].confidential);
    // console.log("this is what will be used to create the datalist", docsNeededForAProperFieldDBDatabase[4]);
    // console.log("this is what will be used to create the session", docsNeededForAProperFieldDBDatabase[5]);
    ...
    if (dontCreateDBsForLearnXUsersBecauseThereAreTooManyOfThem &&
      ...
      console.log(new Date() + "  delaying creation of the dbs for " + corpusDetails.connection.dbname + " and " + activityConnection.dbname + " until they can actually use them.");
       ...
       */
      console.log(new Date() + " Sent command to save user to couch: " + util.inspect(config.usersDbConnection));
...
var emailWelcomeToTheUser = function(user) {
  ...
  if (email && email.length > 5 && config.mailConnection.auth.user !== "") {
    ...
    console.log(new Date() + " Didn't email welcome to new user" + user.username + " why: emailpresent: " + email + ", valid user email: " + (email.length > 5) + ", mailconfig: " + (config.mailConnection.auth.user !== ""));
  }
};
...
var undoCorpusCreation = function(user, connection, docs) {
  ...
  if (email && email.length > 5 && config.mailConnection.auth.user !== "") {
    ...
    console.log(new Date() + " Didnt email welcome to new user" + user.username + " why: emailpresent: " + email + ", valid user email: " + (email.length > 5) + ", mailconfig: " + (config.mailConnection.auth.user !== ""));
  }
...
var addCorpusToUser = function(error, username, newConnection, userPermissionResult, done) {
  ...
  findByUsername(username, function(error, user, info) {
    console.log("Find by username ", info);
    if (error) {
      error.status = cleanErrorStatus(error.status) || 500;
      userPermissionResult.status = error.status;
    ...
    if (userPermissionResult.after.length > 0) {
      ...
      console.log('after removed new corpus from user ', newConnection, user.corpora);
    }
